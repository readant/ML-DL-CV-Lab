...
class MediaPipeTestBase:
    ...
    def draw_connections(self, frame, landmarks, connections, color_map=None):
        ...
        for start_idx, end_idx in connections:
            if start_idx < len(landmarks) and end_idx < len(landmarks):
                ...
                color = color_map.get(start_idx, (0, 255, 255)) if color_map else (0, 255, 255)
    ...
    def draw_ui(self, frame, detected, status_text=""):
        ...
        cv2.putText(frame, self.title, (20, 45), cv2.FONT_HERSHEY_DUPLEX, 1.3, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f"FPS: {self.fps:.1f}", (w - 120, 45), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 255, 255), 2, cv2.LINE_AA)
        ...
        if detected:
            ...
            cv2.putText(frame, f"  No {self.title.replace(' Detection', '').replace(' Recognition', '').replace(' Analysis', '')} Detected", (20, h - 18), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
